...
def validate_job(job: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
    ...
    if not int(config["min_story_characters"]) <= length <= int(config["max_story_characters"]):
    ...
    tags = job["tags"]
    if (
        not isinstance(tags, list) or len(tags) > 30
        or any(not isinstance(tag, str) or not 1 <= len(tag) <= 100 for tag in tags)
        or sum(len(tag) for tag in tags) > 450
    ):
        raise ValidationError("tags must be up to 30 short strings (450 characters total)")
...
def main() -> int:
    ...
    parser.add_argument("--output", default="output/job.json")
    ...
    try:
        ...
        print(f"Validated job {normalized['job_id']} ({len(normalized['story'])} characters)")
        return 0
