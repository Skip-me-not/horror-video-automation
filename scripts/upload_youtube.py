...
def shorts_description(value: str) -> str:
    ...
    return cleaned if "#shorts" in cleaned.casefold() else f"{cleaned}\n\n{hashtags}".strip()
...
def credentials_from_environment() -> Credentials:
    ...
    if missing:
        raise ValidationError(f"missing YouTube secrets: {', '.join(missing)}")
    return Credentials(
        token=None,
...
def upload(video: Path, job: dict, project: Path) -> dict:
    if job.get("privacy_status", "private") != "private":
        raise ValidationError("upload privacy must be private")
    youtube = build("youtube", "v3", credentials=credentials_from_environment(),
    ...
    if thumbnail:
        thumb_path = resolve_asset(project / "assets/thumbnails", thumbnail)
        if not thumb_path.is_file():
            raise FileNotFoundError(f"thumbnail is missing: {thumb_path}")
...
def main() -> int:
    ...
    parser.add_argument("--output", default="output/upload-result.json")
    args = parser.parse_args()
    project = Path(__file__).resolve().parents[1]
    result = upload(Path(args.video), load_json(args.job), project)
